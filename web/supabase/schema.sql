...
create table if not exists events (
  ...
  sweep_plum_density int not null default 800,
  sweep_plum_std_dev int not null default 100,
  sweep_leaf_density int not null default 330,
  sweep_leaf_std_dev int not null default 45,
...
create table if not exists sweep_results (
  ...
  board_items_json jsonb not null default '[]'::jsonb,
  swept_item_ids_json jsonb not null default '[]'::jsonb,
...
create table if not exists random_sources (
  ...
  round_id text not null references rounds(id) on delete cascade,
  participant_id text not null references participants(id) on delete cascade,
  source_type text not null check (source_type in ('sweep_random', 'focused_true', 'distracted_random')),
  n1 int not null,
  n2 int not null,
  hexagram_summary_json jsonb not null,
  created_at timestamptz not null default now(),
...
create table if not exists blind_mappings (
  ...
  round_id text not null references rounds(id) on delete cascade,
  participant_id text not null references participants(id) on delete cascade,
  blind_id text not null check (blind_id in ('A', 'B', 'C')),
  source_type text not null check (source_type in ('sweep_random', 'focused_true', 'distracted_random')),
  created_at timestamptz not null default now(),
